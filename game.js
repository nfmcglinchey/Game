...
class DungeonLevel extends Phaser.Scene {
    ...
    create() {
        ...
        // Keys
        let key = this.physics.add.sprite(500, 250, 'key');
...
class CityLevel extends Phaser.Scene {
    ...
    update() {
        if (isOnBike) {
            ...
            // Fix: Corrected "couchers.up" to "cursors.up"
        ...
        // Mount bike if near it
        if (Phaser.Math.Distance.Between(player.x, player.y, bike.x, bike.y) < 50 && !isOnBike) {
            isOnBike = true;
